...
\PaperTitle{Credit Card Defaulters: Predictive Models} % Article title
...
\Keywords{Credit Card --- Defaulting --- Prevision} % Keywords - if you don't want any simply remove all the text between the curly brackets
...
\Abstract{Credit card default happens when you've become severely delinquent on your credit card payment.
	...
	It's a serious credit card status that not only affects someones standing with that credit card issuer, but also his/her credit standing in general and his/her ability to get approved for credit cards, loans, and other credit-based services\cite{Default}.
	
	When someone accept a credit card, he/she agree to certain terms, e.g. to make your minimum payment by the due date listed on his/her credit card statement.
	
	If he/she miss the minimum credit card payment six months in a row, your credit card will be in default; The credit card issuer will likely close the account and report the default to the credit bureaus.
	
	By the time the credit card defaults, he/she have likely accumulated hundreds of dollars in fees and interest charges. 
	Unfortunately, the options for clearing up the credit card default may be limited because of the number of payments  missed on the account\cite{ItalPress}. 
	
	For this reason, assuming truthful the given data, we show the procedure used to create a prevision algorithm aiming to foresee the default payment's client.
}
...
\begin{table}[h!]
	...
	\end{tabular}}
	\caption{Evaluation using Holdout}
	\label{table:holdNB}
\end{table} 
\pagebreak
\begin{table}[h!]
	...
	\end{tabular}}
	\caption{Evaluation using 3-Folds Cross Validation}
	\label{table:3fcvNB}
\end{table} 
...
\onecolumn{
	...
	%----------- -----------------------------------------------------------------------------
	%	REFERENCE LIST & the workflow overview
	%----------------------------------------------------------------------------------------
