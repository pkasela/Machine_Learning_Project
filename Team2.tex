...
The Tables \ref{table:holdnb}, \ref{table:3fcvNB} are optained after applying the Feature Selection with NaiveBayes. 
The latter shows that AUC measure increases in Bayesian models and others remains more or less unchanged. 
The F$_1$-measure gets worse slightly, but we cannot state anything about the model getting either worse or better. 
...
\begin{table}[h!]
	...
	\label{table:3fcvNB}
\end{table} 
\noindent The benefit from the Feature Selection is that the number of attributes is reduced from 17 to 10, while we don't see any considerable statistical difference between the initial model and the model obtained after the Feature Selection. 
