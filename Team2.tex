...
\PaperTitle{Credit Card Defaulters: Predictive Model} % Article title
...
\Keywords{Credit Card --- Default --- Prevision} % Keywords - if you don't want any simply remove all the text between the curly brackets
...
\section{Alternative Approaches} %approaches (I use also SMOTE[seems good])
...
In the Figure \ref{fig:SMOTE_ROC} we can see that using ROC Curve Gradient Boosted Trees is a better classifier for any \% of FPR.
...
Using SMOTE with 3-Folds Cross Validation confirms the values obtained with Holdout.


\section{Conclusion}

%Human help, we find the half of the defaulter (half are not defaulter) so the workers can concentrate more on them using experience to find out the possible defaulters.
